...
class Dogadjaj{
    ...
    public static function getAll(mysqli $conn){
        $query = "SELECT d.id,d.naslov,d.opis,d.lokacija,d.vremeod,d.vremedo,u.username FROM dogadjaj d INNER JOIN user u on d.user_id=u.id";
        $myArray = array();
        $result = $conn->query($query);
        if($result){
            while($row = $result->fetch_array()){
    ...
    public function update(mysqli $conn){
        $query = "UPDATE dogadjaj SET naslov ='$this->naslov', opis ='$this->opis', lokacija ='$this->lokacija', vremeod='$this->vremeod', vremedo='$this->vremedo', user_id='$this->user_id' WHERE id='$this->id'";
        return $conn->query($query);
    }
